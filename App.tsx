...
const App: React.FC = () => {
  ...
  const handleAutoSolve = () => {
    ...
    // Reset board for a clean solution run
  ...
  const renderGame = () => (
    <div className="flex flex-col h-[100dvh] w-full px-4 pt-[env(safe-area-inset-top,5px)] pb-[env(safe-area-inset-bottom,5px)] overflow-hidden bg-mesh relative justify-between">
      <header className="flex items-center justify-between w-full px-3 py-1.5 border-b border-white/10 bg-slate-900/90 backdrop-blur-3xl rounded-b-[1.5rem] shadow-xl relative z-50">
        <div className="flex-1 flex justify-start">
          <div className="tooltip-container">
            <button onClick={() => { sounds.playSelect(); stopGameTimer(); setScreen('PAUSED'); }} className="p-2 bg-slate-800 rounded-full text-white hover:bg-sky-500 border border-white/20 active:scale-90 shadow-lg transition-all">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M10 9v6m4-6v6" /></svg>
            </button>
            <span className="tooltip-text">Stasis (Pause)</span>
          </div>
        </div>
        <div className="flex-1 text-center">
            <div className="text-[clamp(0.8rem,4vw,1.1rem)] font-black text-white uppercase tracking-tighter italic leading-none">{tier.phaseName}</div>
            <div className={`text-[6px] font-black uppercase tracking-[0.15em] ${tier.color} mt-0.5`}>{tier.name} • LVL {level} • {diskCount} LAYERS</div>
        </div>
        <div className="flex-1 flex justify-end items-center gap-1.5">
          <div className="tooltip-container">
            <button onClick={() => { sounds.playSelect(); setPreviousScreen('PLAYING'); setScreen('SETTINGS'); }} className="p-2 bg-slate-800 rounded-full text-white hover:bg-sky-500 border border-white/20 active:scale-90 shadow-lg transition-all">
              <SettingsIcon className="w-4 h-4" />
            </button>
            <span className="tooltip-text">Settings</span>
          </div>
          <div className="tooltip-container">
            <button onClick={() => { sounds.playSelect(); stopGameTimer(); setScreen('HOME'); }} className="p-2 bg-slate-800 rounded-full text-white hover:bg-sky-500 border border-white/20 active:scale-90 shadow-lg transition-all">
  ...
  const renderSettings = () => (
    <div className="fixed inset-0 bg-slate-950/98 backdrop-blur-3xl z-[600] flex flex-col items-center justify-center p-8 animate-in fade-in duration-500 overflow-y-auto">
      <div className="w-full max-w-md mx-auto flex flex-col items-center py-6">
        ...
        <div className="mt-8 scale-75 opacity-40"><Logo /></div>
      </div>
    </div>
  );
  ...
  const renderPause = () => (
    <div className="fixed inset-0 bg-slate-950/96 backdrop-blur-3xl z-[600] flex items-center justify-center p-8 animate-in fade-in duration-500 overflow-y-auto">
      <div className="text-center w-full max-w-md mx-auto flex flex-col items-center gap-8">
        ...
        <div className="mt-4 scale-75 opacity-30"><Logo /></div>
      </div>
    </div>
  );
